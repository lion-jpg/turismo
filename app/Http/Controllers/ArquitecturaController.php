...
class ArquitecturaController extends Controller
{
    ...
    public function post(Request $request)
    {
        ...
        $data = $request->except('foto_arq');
        $file = $request->file('foto_arq');
        ...
        try {
            ...
            if (isset($uploadedFile[0]['id'])) {
                ...
                $data['foto_arq'] = $uploadedFile[0]['id'];  // Cambiado para usar el ID del archivo
            } else {
                return response()->json(['error' => 'Error al subir la imagen'], 500);
            }
    
            // Envía los datos a la API de Strapi, incluyendo el ID de la imagen
            $response = $client->post('https://backend-culturas.elalto.gob.bo/api/arquitecturas', [
                'json' => [
        ...
        if ($response->getStatusCode() >= 400) {
            ...
            return redirect('admin/arquitectura-viewer')->with('success', 'Datos actualizados correctamente');
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($file) {
            try {
                ...
                if (isset($uploadedFile[0]['id'])) {
                    $validatedData['foto_arq'] = $uploadedFile[0]['id'];
                } else {
        ...
        try {
            $response = $client->put("https://backend-culturas.elalto.gob.bo/api/arquitecturas/$id", [
                'json' => [
                    'data' => $validatedData,
                ],
                'headers' => [
            ...
            // return redirect('admin/arqui')->with('success', 'Datos actualizados correctamente');
            return redirect('admin/arquitectura-viewer')->with('success', 'Datos actualizados correctamente');
