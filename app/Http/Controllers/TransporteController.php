...
class TransporteController extends Controller
{
    ...
    public function post(Request $request)
    {
        ...
        $data = $request->except('foto_tran');
        $file = $request->file('foto_tran');
        ...
        try {
            ...
            if (isset($uploadedFile[0]['id'])) {
                ...
                $data['foto_tran'] = $uploadedFile[0]['id'];  // Cambiado para usar el ID del archivo
            } else {
                return response()->json(['error' => 'Error al subir la imagen'], 500);
            }
    
            // Envía los datos a la API de Strapi, incluyendo el ID de la imagen
            $response = $client->post('https://backend-culturas.elalto.gob.bo/api/transportes', [
                'json' => [
...
public function update(Request $request, $id)
{
    ...
    if ($file) {
        try {
            ...
            if (isset($uploadedFile[0]['id'])) {
                $validatedData['foto_tran'] = $uploadedFile[0]['id'];
            } else {
    ...
    try {
        $response = $client->put("https://backend-culturas.elalto.gob.bo/api/transportes/$id", [
            'json' => [
                'data' => $validatedData,
            ],
            'headers' => [
